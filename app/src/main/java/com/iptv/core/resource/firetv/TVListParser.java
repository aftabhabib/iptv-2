...
final class TVListParser {
     ...
     * 频道分组
     */
    public ChannelGroup[] readChannelGroups(InputStream input) throws IOException {
        List<ChannelGroup> channelGroupList = new ArrayList<ChannelGroup>(GROUP_NAME.length);

        Channel[] channels = readChannels(input);
        for (int i = 0; i < GROUP_NAME.length; i++) {
            ChannelGroup channelGroup = ChannelGroup.create(
                    GROUP_NAME[i], String.valueOf(i + 1), channels);
            channelGroupList.add(channelGroup);
        }

        return channelGroupList.toArray(new ChannelGroup[channelGroupList.size()]);
    }

    /**
     * 读取频道列表
     */
    private Channel[] readChannels(InputStream input) throws IOException {
